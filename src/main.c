...
int main(void)
{
    ...
    setup_battery_indic();
    mpr121_setup(MPR121_ADDRESS, ELE_EN(0b1100), DEBOUNCE_DT(0b111) | DEBOUNCE_DR(0b111), ESI(0) | SFI(0b00), FFI(0b00) | CDC(0b111111));
    ...
    while(1)
    {
      ...
      else
      {
        if(prev_pressed_pads != pressed_pads)
        {
          ...
          if(observer_get_result(&observer))
          {
            oled_set_cursor(0,6);
            oled_write_int(observer_get_result(&observer));
            oled_clear();
